...
void CodeGenTileLangPPL::VisitExpr_(const CallNode *op, std::ostream &os) {
  ...
  auto handle_elementwise_const = [&, this](std::string op_name) {
    ...
    this->PrintIndent();
    this->stream << "scalar_t " << dst << "_scalar_" << dtype << " = {."
                 << dtype << " = " << value << "};\n";
    this->stream << op_name << "( " << dst << ".addr, " << src0 << ".addr, "
                 << dst << "_scalar_" << dtype << ", "
                 << "&" << dst << ".shape, "
  ...
  if (op->op.same_as(builtin::call_extern())) {
    ...
    if (op_name == "ppl.copy") {
      ...
      auto process_copy = [&, this](const tl::RegionOp &src)
        ...
        if (src_buffer.scope() == "global") {
          ...
          for (int i = 0; i < src_ranges.size(); i++) {
            ...
            min_expr += "(" + PrintExpr(sr->min) + ") * " +
                        strides[stride_map[i]] + "+";
          }
      ...
      if (src_dtype != dst_dtype) {
        // LOG(FATAL) << "Unsupported copy from " << src_dtype << " to " <<
        // dst_dtype;
      ...
      ppl_inst += "(" + dst_var_id + ".addr, " + src_var_id + ".addr, &" +
                  ...
                  "(" + src_var_id + ".default_stride ? NULL : &" + src_var_id +
                  ".stride), " + src_dtype + ");\n";
...
void CodeGenTileLangPPL::AddFunction(const PrimFunc &f) {
  ...
  auto default_stride = [this](const std::string &node) {
    ...
    for (int i = 2; i >= 0; i--) {
      buffer_stride[node][i] =
          buf_shape[i + 1] + "*" + buffer_stride[node][i + 1];
    }
  };
  ...
  auto allocate_name = [&, this](const Var &v, int index, int length) {
    ...
    if (shape.size() == 2) {
      // buffer_shape[buffer_node->name] = {1, shape[0].as<IntImmNode>()->value,
      // 1, shape[1].as<IntImmNode>()->value};
    ...
    std::string inst = "__ppl_tensor_info " + rid +
                       " = {.shape = " + vector2string(buf_shape) +
                       ", .stride = NULL, .addr = " + vid +
                       ", .dtype = " + dtype +
                       ", .mode = 2, .align_mode = 0, .size = 1, "
                       ".unsigned_flag = 0, .default_stride = true};\n";
