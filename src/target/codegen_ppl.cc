...
std::string CodeGenTileLangPPL::Finish() {
  ...
  decl_stream << "typedef struct {\n"
              ...
              << "    bool default_stride;\n"
              << "} __ppl_tensor_info;\n\n";
  decl_stream << "extern int tpu_core_index(void);\n";
...
void CodeGenTileLangPPL::VisitStmt_(const tir::ForNode *op) {
  ...
  if (num_cores_ > 1 && vid == "bx") {
    // Multi-core dispatch: host passes per-core bx_id via API struct
    stream << "{\n";
    stream << "int bx_total = " << extent << ";\n";
    stream << "int bx_per_core = (bx_total + " << num_cores_ << " - 1) / " << num_cores_ << ";\n";
    stream << "int bx_start = tpu_core_index() * bx_per_core;\n";
    stream << "int bx_end = bx_start + bx_per_core;\n";
    stream << "if (bx_end > bx_total) bx_end = bx_total;\n";
    stream << "if (tpu_core_index() < " << num_cores_ << " && bx_start < bx_total) {\n";
    stream << "for (";
    PrintType(op->loop_var.dtype(), stream);
    stream << ' ' << vid << " = bx_start; " << vid << " < bx_end; ++" << vid << ") {\n";
...
void CodeGenTileLangPPL::VisitExpr_(const FloorDivNode *op, std::ostream &os) {
...
void CodeGenTileLangPPL::VisitExpr_(const CallNode *op, std::ostream &os) {
  ...
  if (op->op.same_as(builtin::call_extern())) {
    ...
    if (op_name == "ppl.copy") {
      ...
      auto add_result = Downcast<Bool>(op->args[9])->value;
      ...
      if (!trans_B) {
        this->stream << "tpu_bdc_fp_mm(" << c_access_data << ".addr, "
                     ...
                     << left_right_dtype << ", " << (add_result ? "true" : "false")
                     << ");\n";
      } else {
 ...
 else if (op_name == "ppl.dequant_gptq") {
      auto out_var =
          var_idmap_[op->args[1].as<CallNode>()->args[1].as<VarNode>()];
      auto w_var =
          var_idmap_[op->args[2].as<CallNode>()->args[1].as<VarNode>()];
      ...
      auto out_dtype = op->args[1].as<CallNode>()->args[0].as<CallNode>()->dtype;
      std::string dtype;
      if (out_dtype == DataType::Float(16)) {
        dtype = "DT_FP16";
      } else if (out_dtype == DataType::Float(32)) {
        dtype = "DT_FP32";
      } else if (out_dtype == DataType::BFloat(16)) {
        dtype = "DT_BFP16";
      } else {
        LOG(FATAL) << "Unsupported dtype in ppl.dequant_gptq: " << out_dtype;
      }

      auto skip_zp = Downcast<Bool>(op->args[10])->value;

      this->PrintIndent();
      int sid = this->BeginScope();
      this->stream << "{\n";

      // Step 1: cast uint4-packed weight -> fp16
      this->PrintIndent();
      this->stream << "tpu_bdc_cast(" << out_var << ".addr, " << w_var
                   << ".addr, "
                   << "&" << out_var << ".shape, "
                   << "(" << out_var << ".default_stride ? NULL : &" << out_var
                   << ".stride), "
                   << "(" << w_var << ".default_stride ? NULL : &" << w_var
                   << ".stride), "
                   << dtype << ", DT_UINT4, RM_HALF_TO_EVEN);\n";

      if (!skip_zp) {
        // Step 2a: cast packed zp (uint8 -> fp16, DT_UINT4 unpacks nibbles)
        this->PrintIndent();
        this->stream << "tpu_bdc_cast(" << work_var << ".addr, " << zp_var
                     << ".addr, "
                     << "&" << work_var << ".shape, "
                     << "(" << work_var << ".default_stride ? NULL : &" << work_var
                     << ".stride), "
                     << "(" << zp_var << ".default_stride ? NULL : &" << zp_var
                     << ".stride), "
                     << dtype << ", DT_UINT4, RM_HALF_TO_EVEN);\n";

        // Step 2b: compute broadcast stride for work buffer (zp broadcast)
        this->PrintIndent();
        this->stream << "dim4 " << work_var << "_bc_stride;\n";
        this->PrintIndent();
        this->stream << "tpu_aligned_stride(&" << work_var << "_bc_stride, 0, &"
                     << work_var << ".shape, " << dtype << ");\n";
        this->PrintIndent();
        this->stream << work_var << "_bc_stride.w = 0;\n";

        // Step 2c: element-wise sub zp (broadcast per-group zp)
        this->PrintIndent();
        this->stream << "tpu_bdc_fp_sub(" << out_var << ".addr, " << out_var
                     << ".addr, " << work_var << ".addr + ((" << PrintExpr(op->args[11]) << " % 2) * 2)"
                     << ", &" << out_var << ".shape, ("
                     << out_var << ".default_stride ? NULL : &" << out_var
                     << ".stride), ("
                     << out_var << ".default_stride ? NULL : &" << out_var
      ...
      // Step 3: compute broadcast stride for scale
      this->PrintIndent();
      this->stream << "dim4 " << scale_var << "_bc_stride;\n";
      this->PrintIndent();
      this->stream << "tpu_aligned_stride(&" << scale_var << "_bc_stride, 0, &"
                   << scale_var << ".shape, " << dtype << ");\n";
      this->PrintIndent();
      this->stream << scale_var << "_bc_stride.w = 0;\n";

      // Step 4: element-wise mul scale (broadcast per-group scale)
      this->PrintIndent();
      this->stream << "tpu_bdc_fp_mul(" << out_var << ".addr, " << out_var
                   << ".addr, " << scale_var << ".addr, &" << out_var
                   << ".shape, ("
                   << out_var << ".default_stride ? NULL : &" << out_var
                   << ".stride), ("
                   << out_var << ".default_stride ? NULL : &" << out_var
                   << ".stride), &"
                   << scale_var << "_bc_stride, "
                   << dtype << ");\n";
