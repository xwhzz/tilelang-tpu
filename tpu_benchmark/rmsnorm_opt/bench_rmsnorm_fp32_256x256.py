"""
Benchmark: optimized RMSNorm FP32 256x256 vs PPL.

Fixed TL variant: `reload_b256`.
...
def tl_rmsnorm_opt(M, N, blk_m):
    ...
    def main_kernel_inner(
        ...
        with T.Kernel(T.ceildiv(M, blk_m), is_cpu=True) as (bx,):
            ...
            T.ppl_mul(A_shared, A_shared, A_shared)
            T.ppl_reduce_sum(A_shared, A_powsum, dim=1)
            T.ppl_mul_C(A_powsum, A_powsum, reciprocal_N)
            T.ppl_add_C(A_powsum, A_powsum, T.float32(EPS))
...
def main():
    ...
    print("=" * 72)
    print(f"RMSNorm FP32 optimized  M={M} N={N}  variant={VARIANT}  BLK_M={BLK_M}")
    print("=" * 72, flush=True)
