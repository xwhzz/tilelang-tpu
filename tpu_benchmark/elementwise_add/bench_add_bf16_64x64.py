...
Usage:
    ...
    python tpu_benchmark/elementwise_add/bench_add_bf16_64x64.py
"""
...
def tl_add_bf16(M, N, block_M, block_N):
    dtype = "bfloat16"
...
def main():
    ...
    tl_kernel = tilelang.compile(
        tl_add_bf16(M, N, BLOCK_M, BLOCK_N), out_idx=-1, target="tpu")
    run_and_check("tilelang", tl_kernel, a, b, ref)

    print("\n--- PPL ---")
    try:
        from ppl_utils import compile_ppl_kernel
        pl_path = os.path.join(BENCH_DIR, "pl", "add_bf16_64x64.pl")
