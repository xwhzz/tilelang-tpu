...
__KERNEL__ void main_kernel(fp16 *ptr_a, fp16 *ptr_b, fp16 *ptr_c) {
  ...
  auto g_a = gtensor<fp16>(global_shape, GLOBAL, ptr_a);
  auto g_b = gtensor<fp16>(global_shape, GLOBAL, ptr_b);
  auto g_c = gtensor<fp16>(global_shape, GLOBAL, ptr_c);
  ...
  for (int idx_c = 0; idx_c < C; idx_c += block_c) {
    ...
    for (int idx_w = 0; idx_w < W; idx_w += block_w) {
      ...
      auto a = make_tensor<fp16>(block_shape, in_shape);
      auto b = make_tensor<fp16>(block_shape, in_shape);
