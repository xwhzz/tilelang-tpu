...
__KERNEL__ void main_kernel(fp16 *ptr_a, fp16 *ptr_b, fp16 *ptr_c) {
  int M = 64;
  int K = 64;
  int N = 64;
  int block_m = 32;
  ...
  auto g_a = gtensor<fp16>(a_global_shape, GLOBAL, ptr_a);
  auto g_b = gtensor<fp16>(b_global_shape, GLOBAL, ptr_b);
  auto g_c = gtensor<fp16>(c_global_shape, GLOBAL, ptr_c);
  ...
  for (int idx_m = 0; idx_m < M; idx_m += block_m) {
    ...
    for (int idx_n = 0; idx_n < N; idx_n += block_n) {
      ...
      for (int idx_k = 0; idx_k < K; idx_k += block_k) {
        ...
        auto a_local = make_tensor<fp16>(a_block_shape, a_shape);
        dma::load(a_local, g_a.sub_view(a_shape, a_offset));
        auto a_fp16 = a_local;
        auto b_local = make_tensor<fp16>(b_block_shape, b_shape);
        dma::load(b_local, g_b.sub_view(b_shape, b_offset));
        auto b_fp16 = b_local;
        tiu::fmm2(c_local, a_fp16, b_fp16, true, DT_FP32);
      }
      auto c_cast = make_tensor<fp16>(c_block_shape, c_shape);
