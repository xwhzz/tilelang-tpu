"""
Benchmark: RoPE FP16 128x64 — tilelang vs PPL

Usage:
    cd /mnt2/users/tilelanguser-xxw/tilelang-tpu
    python tpu_benchmark/rope/bench_rope_fp16_128x64.py
"""
...
def tl_rope_fp16(Block_c, Block_w, C, W):
    dtype = "float16"
    accum_dtype = "float16"
    global_shape = (C, W)
...
def run_and_check(name, kernel_func, x, cos_val, sin_val, ref):
    out = torch.zeros(C, W, dtype=torch.float16)
...
def make_rope_cos_sin(C, W, dtype=torch.float16):
    half = W // 2
...
def main():
    x = torch.randn(C, W, dtype=torch.float16)
    cos_val, sin_val = make_rope_cos_sin(C, W)
    ref = rope_ref(x, cos_val, sin_val)

    print("=" * 60)
    print(f"RoPE FP16  C={C} W={W}  block={BLOCK_C}x{BLOCK_W}")
    print("=" * 60)

    print("\n--- tilelang ---")
    tl_kernel = tilelang.compile(
        tl_rope_fp16(BLOCK_C, BLOCK_W, C, W), out_idx=-1, target="tpu")
    run_and_check("tilelang", tl_kernel, x, cos_val, sin_val, ref)

    print("\n--- PPL ---")
    try:
        from ppl_utils import compile_ppl_kernel
        pl_path = os.path.join(BENCH_DIR, "pl", "rope_fp16_128x64.pl")
        arg_specs = [((C, W), torch.float16)] * 4
