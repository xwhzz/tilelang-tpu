...
// RoPE bf16: all compute in bf16
// Parameter order: (G_in, G_cos, G_sin, G_out) matching tilelang
__KERNEL__ void main_kernel(bf16 *ptr_in, bf16 *ptr_cos, bf16 *ptr_sin,
                            bf16 *ptr_out) {
  int C = 128;
  int W = 64;
  int block_w = 16;
  ...
  auto g_in = gtensor<bf16>(global_shape, GLOBAL, ptr_in);
  auto g_out = gtensor<bf16>(global_shape, GLOBAL, ptr_out);
  auto g_cos = gtensor<bf16>(global_shape, GLOBAL, ptr_cos);
  auto g_sin = gtensor<bf16>(global_shape, GLOBAL, ptr_sin);
  ...
  for (int idx_c = 0; idx_c < C; idx_c += block_c) {
    ...
    for (int idx_w = 0; idx_w < W; idx_w += block_w) {
      ...
      auto in = make_tensor<bf16>(block_shape, in_shape);
      auto in_cos = make_tensor<bf16>(block_shape, in_shape);
      auto in_sin = make_tensor<bf16>(block_shape, in_shape);
      ...
      auto x_cos = make_tensor<bf16>(block_shape, in_shape);
      tiu::fmul(x_cos, in, in_cos);

      auto x_sin = make_tensor<bf16>(block_shape, in_shape);
      tiu::fmul(x_sin, in, in_sin);

      auto x_neg = make_tensor<bf16>(block_shape, in_shape);
      tiu::fmul(x_neg, in, -1.0);
      auto x_neg_sin = make_tensor<bf16>(block_shape, in_shape);
      ...
      dim4 half_stride;
      get_stride(&half_stride, &in_shape, TPU_ALIGN, get_eu_num<bf16>());
      half_stride.w = 2;
      dim4 offset = {0, 0, 0, 1};
      auto x_neg_sin_half =
