...
// Parameter order matches tilelang: (G_in, G_cos, G_sin, G_out)
__KERNEL__ void main_kernel(fp32 *ptr_in, fp32 *ptr_cos, fp32 *ptr_sin,
                            fp32 *ptr_out) {
  int C = 128;
  int W = 64;
  int block_w = 16;
  ...
  auto g_cos = gtensor<fp32>(global_shape, GLOBAL, ptr_cos);
  auto g_sin = gtensor<fp32>(global_shape, GLOBAL, ptr_sin);
  ...
  for (int idx_c = 0; idx_c < C; idx_c += block_c) {
    ...
    for (int idx_w = 0; idx_w < W; idx_w += block_w) {
      ...
      auto in_cos = make_tensor<fp32>(block_shape, in_shape);
      auto in_sin = make_tensor<fp32>(block_shape, in_shape);
      ...
      // x_cos = x * cos
      auto x_cos = make_tensor<fp32>(block_shape, in_shape);
      tiu::fmul(x_cos, in, in_cos);

      // x_sin = x * sin
      auto x_sin = make_tensor<fp32>(block_shape, in_shape);
      tiu::fmul(x_sin, in, in_sin);

      // -x
      auto x_neg = make_tensor<fp32>(block_shape, in_shape);
      tiu::fmul(x_neg, in, -1.0);
      auto x_neg_sin = make_tensor<fp32>(block_shape, in_shape);
      ...
      dim4 half_stride;
      get_stride(&half_stride, &in_shape, TPU_ALIGN, get_eu_num<fp32>());
      half_stride.w = 2;
      dim4 offset = {0, 0, 0, 1};
      auto x_neg_sin_half =
