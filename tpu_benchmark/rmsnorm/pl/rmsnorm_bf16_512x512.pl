...
__KERNEL__ void main_kernel(bf16 *ptr_input, bf16 *ptr_output) {
  int M = 512;
  int N = 512;
  int block_m = 64;
  float eps = 1e-12f;
  ...
  auto g_in = gtensor<bf16>(global_shape, GLOBAL, ptr_input);
  auto g_out = gtensor<bf16>(global_shape, GLOBAL, ptr_output);
  ...
  for (int idx_m = 0; idx_m < M; idx_m += block_m) {
    ...
    auto in_local = make_tensor<bf16>(block_shape, in_shape);
    ...
    auto out_cast = make_tensor<bf16>(block_shape, in_shape);
