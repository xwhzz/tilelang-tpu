...
__KERNEL__ void main_kernel(fp32 *ptr_input, fp32 *ptr_output) {
  int M = 64;
  int N = 64;
  int block_m = 64;
  float eps = 1e-12f;
  ...
  auto g_in = gtensor<fp32>(global_shape, GLOBAL, ptr_input);
  auto g_out = gtensor<fp32>(global_shape, GLOBAL, ptr_output);
