"""
Benchmark: RMSNorm BF16 1024x1024 — tilelang (optimized) vs PPL
...
def tl_rmsnorm_lowp(M, N, blk_m):
    ...
    def main_kernel_inner(
        ...
        with T.Kernel(T.ceildiv(M, blk_m), is_cpu=True) as (bx,):
            ...
            T.ppl_mul(A_shared_fp32, A_shared_fp32, A_shared_fp32)
            T.ppl_reduce_sum(A_shared_fp32, A_powsum, dim=1)
...
def run_and_check(name, kernel_func, a, ref):
    b = torch.zeros(M, N, dtype=torch.bfloat16)
    kernel_func(a, b)
...
def main():
    ...
    print("=" * 60)
    print(f"RMSNorm BF16 (opt)  M={M} N={N}  blk_m={BLK_M}")
    print("=" * 60)
    ...
    try:
        ...
        pl_path = os.path.join(BENCH_DIR, "pl", "rmsnorm_bf16_1024x1024.pl")
        arg_specs = [((M, N), torch.bfloat16), ((M, N), torch.bfloat16)]
