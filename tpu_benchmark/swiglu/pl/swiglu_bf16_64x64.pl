...
// SwiGLU bf16: input/output bf16, compute in fp32
// Parameter order: (G_in, G_right, G_out) matching tilelang
__KERNEL__ void main_kernel(bf16 *ptr_in, bf16 *ptr_right, bf16 *ptr_out) {
  int C = 64;
  int W = 64;
  int block_w = 16;
  int block_c = 64;
  ...
  auto g_right = gtensor<bf16>(global_shape, GLOBAL, ptr_right);
  ...
  for (int idx_c = 0; idx_c < C; idx_c += block_c) {
    ...
    for (int idx_w = 0; idx_w < W; idx_w += block_w) {
      ...
      auto in_bf16 = make_tensor<bf16>(block_shape, in_shape);
      auto right_bf16 = make_tensor<bf16>(block_shape, in_shape);

      dim4 in_offset = {0, idx_c, 0, idx_w};
      dma::load(in_bf16, g_in.sub_view(in_shape, in_offset));
      dma::load(right_bf16, g_right.sub_view(in_shape, in_offset));

      // cast to fp32
      auto in = make_tensor<fp32>(block_shape, in_shape);
      auto right = make_tensor<fp32>(block_shape, in_shape);
      tiu::cast(in, in_bf16);
      tiu::cast(right, right_bf16);
      ...
      // cast back to bf16
      auto out_bf16 = make_tensor<bf16>(block_shape, in_shape);
      tiu::cast(out_bf16, out_fp32);
      dma::store(g_out.sub_view(in_shape, in_offset), out_bf16);
    }
  }
}
