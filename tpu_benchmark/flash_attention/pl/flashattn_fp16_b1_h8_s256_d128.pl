...
__KERNEL__ void main_kernel(fp16 *ptr_q, fp16 *ptr_k, fp16 *ptr_v,
                            fp16 *ptr_out) {
  int batch = 1;
  int heads = 8;
  int seq_len = 256;
  ...
  auto q_global = make_gtensor_permute<fp16>(qo_global_shape, GLOBAL, ptr_q, order);
  auto k_global = make_gtensor_permute<fp16>(qo_global_shape, GLOBAL, ptr_k, order);
  auto v_global = make_gtensor_permute<fp16>(qo_global_shape, GLOBAL, ptr_v, order);
  auto out_global = make_gtensor_permute<fp16>(qo_global_shape, GLOBAL, ptr_out, order);
  ...
  for (int _b = 0; _b < batch; _b++) {
    for (int _h = 0; _h < heads; _h += block_h) {
      ...
      for (int _m = 0; _m < seq_len; _m += block_m) {
        ...
        tensor<fp16> qi_tensor;
        dma::load(qi_tensor, q_global.sub_view(qi_real_shape, qi_offset));
        tensor<fp16> qi_scaled;
        tiu::fmul(qi_scaled, qi_tensor, sqrt_d);

        dim4 mi_real = {real_h, real_m, 1, 1};
        dim4 acc_real = {real_h, real_m, 1, dim};
        auto mi_sub = make_tensor<fp16>(mi_shape, mi_real);
        auto li_sub = make_tensor<fp16>(mi_shape, mi_real);
        auto acc_sub = make_tensor<fp16>(acc_shape, acc_real);
        ...
        for (int _k = 0; _k < seq_len; _k += block_k_iter) {
          ...
          tensor<fp16> ki_tensor, vi_tensor;
          ...
          auto qk_sub = make_tensor<fp16>(qk_shape, qk_real);
          ...
          tensor<fp16> sub_out1, p_T, sum_val;
          ...
          auto pv = make_tensor<fp16>(acc_shape, acc_real);
        ...
        tensor<fp16> div_li_T, final_out;
