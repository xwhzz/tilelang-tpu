"""
FlashAttention FP16 B1H8S512D128 — tilelang vs PPL
...
BATCH, HEADS, SEQ_LEN, DIM = 1, 8, 512, 128
...
def tl_flashattn_fp16_v2(batch, heads, seq_len, dim, block_M, block_N,
...
def ref_program(Q, K, V, is_causal):
    ...
    return torch.einsum('bhqk,bkhd->bqhd', F.softmax(scores, dim=-1), V.float()).half()


def run_and_check(name, kernel_func, q, k, v, ref):
    out = torch.zeros(BATCH, SEQ_LEN, HEADS, DIM, dtype=torch.float16)
...
def main():
    ...
    print("=" * 60)
    print(f"FlashAttention FP16 v2  B={BATCH} H={HEADS} S={SEQ_LEN} D={DIM}")
    print("=" * 60)

    print("\n--- tilelang ---")
    tl_kernel = tilelang.compile(
        tl_flashattn_fp16_v2(BATCH, HEADS, SEQ_LEN, DIM, BLOCK_M, BLOCK_N,
                             IS_CAUSAL, NUM_STAGES),
    ...
    try:
        ...
        pl_path = os.path.join(BENCH_DIR, "pl", "flashattn_fp16_b1_h8_s512_d128.pl")
        qkv_shape = (BATCH, SEQ_LEN, HEADS, DIM)
        arg_specs = [(qkv_shape, torch.float16)] * 4
