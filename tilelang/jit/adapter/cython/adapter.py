...
_TPU_RUNTIME_LOCK = threading.RLock()


def _destroy_tpu_context(lib: ctypes.CDLL, context: int) -> None:
    with _TPU_RUNTIME_LOCK:
        lib.tilelang_tpu_destroy(context)
...
# === TL TPU C1 CORE PARALLEL PATCH START ===
def _tl_tpu_patch_core_parallel_by_loop():
    return
    """Temporary TPU core-parallel prototype.

    Convert generated by-loop:
        int core_idx = tpu_workitem_index();
        for (int by = core_idx; by < N; by += 8) {
    ...
    Also set kernel.cpp launch config to 8 workitems.

    Enable with:
        export TL_TPU_ENABLE_CORE_PARALLEL=1
    """
    import os
    import re
    from pathlib import Path

    if os.environ.get("TL_TPU_ENABLE_CORE_PARALLEL_M1024", "0") != "1":
        return

    repo = Path(__file__).resolve().parents[4]
    tpu_dir = repo / "src" / "tl_templates" / "tpu"

    kernel_c = tpu_dir / "kernel.c"
    kernel_cpp = tpu_dir / "kernel.cpp"

    # if kernel_c.exists():
    #     text = kernel_c.read_text()

    #     if "tpu_workitem_index()" not in text:
    #         pattern = r"for \(int by = 0; by < ([0-9]+); \+\+by\) \{"

    #         def repl(match):
    #             by_blocks = match.group(1)
    #             return (
    #                 "int core_idx = tpu_workitem_index();\n"
    #                 f"    for (int by = core_idx; by < {by_blocks}; by += 8) {{"
    #             )

    #         text, count = re.subn(pattern, repl, text, count=1)

    #         if count == 0:
    #             print("[TL TPU C1 PATCH] no by-loop matched", flush=True)
    #         else:
    #             print("[TL TPU C1 PATCH] patched by-loop to core parallel", flush=True)

    #         kernel_c.write_text(text)

    if kernel_cpp.exists():
        text = kernel_cpp.read_text()

        text = text.replace("int core_num = 1;", "int core_num = 8;", 1)
        text = text.replace("int block_num = 1;", "int block_num = 8;", 1)

        kernel_cpp.write_text(text)

def _normalize_tpu_core_parallel_config(config):
    """Normalize TPU core-parallel runtime option."""
    if config is None:
        return {
            "enable": False,
            "loop": "by",
...
def _temporary_tpu_core_parallel_env(config):
    """Temporarily set TPU core-parallel environment variables during runtime call."""
    config = _normalize_tpu_core_parallel_config(config)
...
# === TL TPU C1 CORE PARALLEL PATCH END ===
class CythonKernelAdapter(BaseKernelAdapter):
    ...
    def __init__(self,
        ...
        self.mode = mode
        self.tpu_core_parallel = _normalize_tpu_core_parallel_config(tpu_core_parallel)
        if is_tpu_target(self.target):
            self.tpu_device_id = int(tpu_device_id)
            self.tpu_runtime_root = tpu_runtime_root
            artifact_key = hashlib.sha256((
                self.kernel_global_source
                + self.mode
                + repr(self.tpu_core_parallel)
                + str(self.tpu_runtime_root)
            ).encode()).hexdigest()[:20]
            artifact_root = Path(TILELANG_CACHE_DIR) / "tpu" / self.mode
            artifact_root.mkdir(parents=True, exist_ok=True)
            self.artifact_dir = tempfile.mkdtemp(
                prefix=f"{artifact_key}-", dir=artifact_root)
            self.wrapper = TLWrapper(self.target, artifact_dir=self.artifact_dir)
        ...
        if is_tpu_target(self.target):
            pass  # adapter patch disabled; codegen_ppl.cc handles core-parallel
        self.lib_generator.compile_lib()
        self.lib = self.lib_generator.load_lib()
        if is_tpu_target(self.target):
            self.lib.tilelang_tpu_create.argtypes = [ctypes.c_char_p, ctypes.c_int]
            self.lib.tilelang_tpu_create.restype = ctypes.c_void_p
            self.lib.tilelang_tpu_run.argtypes = [
                ctypes.c_void_p, ctypes.POINTER(ctypes.c_void_p)
            ]
            self.lib.tilelang_tpu_run.restype = ctypes.c_int
            self.lib.tilelang_tpu_destroy.argtypes = [ctypes.c_void_p]
            self.lib.tilelang_tpu_destroy.restype = None
            self.lib.tilelang_tpu_last_error.argtypes = [ctypes.c_void_p]
            self.lib.tilelang_tpu_last_error.restype = ctypes.c_char_p

            kernel_path = os.path.join(self.artifact_dir, "libkernel.so")
            with _TPU_RUNTIME_LOCK:
                self.tpu_context = self.lib.tilelang_tpu_create(
                    os.fsencode(kernel_path), self.tpu_device_id)
            if not self.tpu_context:
                raw_error = self.lib.tilelang_tpu_last_error(None)
                error = raw_error.decode() if raw_error else "unknown TPU initialization error"
                raise RuntimeError(
                    f"Failed to initialize TPU runtime in {self.mode} mode on "
                    f"device {self.tpu_device_id}: {error}")
            self._tpu_finalizer = weakref.finalize(
                self, _destroy_tpu_context, self.lib, self.tpu_context)

            def lambda_forward(*args):
                implicit_outputs = len(args) != len(self.params)
                if implicit_outputs:
                    expected_inputs = len(self.params) - len(self.result_idx)
                    if len(args) != expected_inputs:
                        raise TypeError(
                            f"Expected {expected_inputs} input tensors or {len(self.params)} "
                            f"explicit tensors, got {len(args)}")
                    supplied = iter(args)
                    full_args = []
                    output_device = args[0].device if args else torch.device("cpu")
                    for index, param in enumerate(self.params):
                        if index in self.result_idx:
                            if not all(isinstance(dim, int) for dim in param.shape):
                                raise ValueError(
                                    "Implicit TPU outputs currently require static shapes")
                            full_args.append(torch.empty(
                                tuple(param.shape), dtype=param.dtype, device=output_device))
                        else:
                            full_args.append(next(supplied))
                else:
                    full_args = list(args)

                host_tensors = [arg.detach().cpu().contiguous() for arg in full_args]
                arg_buffers = []
                arg_sizes = []
                ...
                argv = (ctypes.c_void_p * len(full_args))()
                ...
                with _temporary_tpu_core_parallel_env(self.tpu_core_parallel):
                    with _TPU_RUNTIME_LOCK:
                        ret = self.lib.tilelang_tpu_run(self.tpu_context, argv)

                if ret != 0:
                    raw_error = self.lib.tilelang_tpu_last_error(self.tpu_context)
                    error = raw_error.decode() if raw_error else f"status {ret}"
                    raise RuntimeError(f"TPU kernel execution failed: {error}")

                args_list = full_args
                for i in self.result_idx:
                    result_tensor = torch.empty(
                        full_args[i].shape, dtype=full_args[i].dtype, device="cpu")
                    ctypes.memmove(
                ...
                if implicit_outputs:
                    outputs = [args_list[i] for i in self.result_idx]
