...
def rms_norm_splitk(M, N, blk_m, blk_k, dtype="float16"):
    ...
    def main_kernel_inner(
        ...
        with T.Kernel(T.ceildiv(M, blk_m), is_cpu=True) as (bx,):
            ...
            A_shared_fp32 = T.alloc_shared((blk_m, blk_k), "float32")
...
kernel = tilelang.compile(
    rms_norm_splitk(M, N, blk_m, blk_k), out_idx=-1, target="tpu")

a = torch.randn(M, N).half()
b = torch.zeros(M, N).half()
