...
def rope(Block_c, Block_w, C, W, dtype="float16", accum_dtype="float16"):
...
def make_rope_cos_sin(C, W, base=10000.0, pos_start=0, device="cpu", dtype=torch.float16):
...
# cos = read_txt_per_line("tpu_demo/cos.txt", C, W, dtype=torch.float16)
# sin = read_txt_per_line("tpu_demo/sin.txt", C, W, dtype=torch.float16)
cos, sin = make_rope_cos_sin(C, W, device=x.device, dtype=x.dtype)
out_kernel = torch.zeros(C, W, dtype=torch.float16)

# 运行 kernel
res = kernel(x, cos, sin, out_kernel)

print(res)
out_ref = rope_ref_torch(x, cos, sin)
