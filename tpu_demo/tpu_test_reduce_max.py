...
def reduce_max(M, N, block_M, block_N, dtype="float16", accum_dtype="float16"):
    @T.prim_func
    def main_kernel_inner(
        ...
        with T.Kernel(1, T.ceildiv(M, block_M), is_cpu=True) as (bx, by):
            ...
            T.reduce_max(X_shared, Y_shared, 1, True)
            # 将结果从共享内存复制回全局内存
...
kernel = tilelang.compile(reduce_max(M, N, block_M, block_N), out_idx=-1, target="tpu")

x = torch.randn(M, N).half()
y = torch.zeros(M, 1).half()

res = kernel(x, y)
y_torch = torch.max(x, dim=1, keepdim=True).values
