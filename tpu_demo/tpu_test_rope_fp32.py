...
# cos = read_txt_per_line("tpu_demo/cos.txt", C, W, dtype=torch.float32)
# sin = read_txt_per_line("tpu_demo/sin.txt", C, W, dtype=torch.float32)
cos, sin = make_rope_cos_sin(C, W, device=x.device, dtype=x.dtype)
out_kernel = torch.zeros(C, W, dtype=torch.float32)

# 运行 kernel
res = kernel(x, cos, sin, out_kernel)

print(res)
out_ref = rope_ref_torch(x, cos, sin).float()
