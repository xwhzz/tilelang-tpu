...
def llama2_flash_attn_tpu(B, H, L, D, causal=False):
    scale = (1.0 / D) ** 0.5 * 1.44269504  # log2(e)
    shape = [B, L, H, D]
    ...
    def kernel_func(block_M, block_N, num_stages, threads):
        ...
        def MMA_QK(
            ...
            Q_shared: T.Tensor([block_M, D], dtype),
            K_shared: T.Tensor([block_N, D], dtype),
        ...
        def MMA_SV(
            ...
            V_shared: T.Tensor([block_N, D], dtype),
            acc_s_cast: T.Tensor([block_M, block_N], dtype),
            acc_o: T.Tensor([block_M, D], accum_dtype),
            k: T.int32,
            by: T.int32,
            bz: T.int32,
        ):
        ...
        def main(
            ...
            with T.Kernel(T.ceildiv(L, block_M), H, B, is_cpu=True) as (bx, by, bz):
                ...
                loop_range = (
                    T.min(T.ceildiv(L, block_N), T.ceildiv((bx + 1) * block_M, block_N)) if causal
                    else T.ceildiv(L, block_N)
                )

                for k in T.Pipelined(loop_range, num_stages=num_stages):
                    MMA_QK(K, Q_shared, K_shared, acc_s, k, bx, by, bz)
                    # Optional causal mask: mask future positions
                    if T.meta_var("causal"):
...
from tpu_demo.ppl.flashattn.llama2_flash_attn_tpu import llama2_flash_attn_tpu
import tilelang

B, H, L, D = 2, 32, 2048, 128
block_M, block_N, num_stages, threads = 128, 64, 2, 128
func = llama2_flash_attn_tpu(B, H, L, D, causal=False)(block_M, block_N, num_stages, threads)
