...
def test_exp(M, N, block_M, block_N, accum_dtype):
    ...
    @T.prim_func
    def main_kernel_inner(scores_scale: T.Tensor([M, N], accum_dtype)):
        with T.Kernel(T.ceildiv(N, block_N), T.ceildiv(M, block_M), is_cpu=True) as (bx, by):
            ...
            T.copy(scores_scale[by * block_M, bx * block_N], scores_scale_shared)
            T.exp(scores_scale_shared)
            T.copy(scores_scale_shared, scores_scale[by * block_M, bx * block_N])
