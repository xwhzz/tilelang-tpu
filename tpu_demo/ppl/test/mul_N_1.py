...
def rms_norm(M, N, blk_m, dtype):
    ...
    @T.prim_func
    def main_kernel_inner(C: T.Tensor((M, N), dtype), A: T.Tensor((M, N), dtype), B: T.Tensor((M, 1), dtype)):
        ...
        with T.Kernel(T.ceildiv(M, blk_m), is_cpu=True) as (bx,):
            ...
            T.copy(B[bx * blk_m:(bx + 1) * blk_m, 0], B_shared)
            T.mul(A_shared, A_shared, B_shared)
            T.copy(A_shared, C[bx * blk_m:(bx + 1) * blk_m, :])
