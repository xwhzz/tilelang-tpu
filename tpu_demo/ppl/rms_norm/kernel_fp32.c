...
void rms_norm_inner(global_addr_t v1, global_addr_t v2) {
  ...
  __ppl_tensor_info A_pow2 = {.shape = { 1, 64, 1, 2048}, .stride = NULL, .addr = 0, .dtype = DT_FP32, .mode = 2, .align_mode = 1, .size = 8192, .unsigned_flag = 0, .default_stride = true};
  __ppl_tensor_info A_powsum = {.shape = { 1, 64, 1, 1}, .stride = NULL, .addr = 114688, .dtype = DT_FP32, .mode = 2, .align_mode = 1, .size = 4, .unsigned_flag = 0, .default_stride = true};
  __ppl_tensor_info tmp_buffer_sum = {.shape = { 1, 64, 1, 32}, .stride = NULL, .addr = 98304, .dtype = DT_FP32, .mode = 2, .align_mode = 1, .size = 128, .unsigned_flag = 0, .default_stride = true};
  for (int bx = 0; bx < 32; ++bx) {
    __ppl_tensor_info A = {.shape = {1, 64, 1, 2048}, .stride = {4194304, 2048, 2048, 1} , .addr = v3.addr + (((bx * 64)) * 2048+(0) * 1 ) * 4, .dtype = DT_FP32, .mode = 2, .size = 1, .offset = (((bx * 64)) * 2048+(0) * 1 ) * 4, .unsigned_flag = 0, .default_stride = false};
    __ppl_tensor_info A_shared_1 = {.shape = {1, 64, 1, 2048}, .stride = NULL, .addr = A_shared.addr, .dtype = DT_FP32, .mode = 0, .size = 1, .offset = 0, .unsigned_flag = 0, .default_stride = true};
    ...
    tpu_bdc_fp_mul( A_pow2.addr, A_shared.addr, A_shared.addr, &A_pow2.shape, (A_pow2.default_stride ? NULL : &A_pow2.stride), (A_shared.default_stride ? NULL : &A_shared.stride), (A_shared.default_stride ? NULL : &A_shared.stride), DT_FP32);
    {
      ...
      if (align_w > A_pow2.shape.w) {
        ...
        dim4 fill_tensor_stride = {2048, align_w, A_pow2.shape.w, 1};
    ...
    tpu_bdc_fp_mul_C( A_powsum.addr, A_powsum.addr, (scalar_t){.f32 = 0.000488281}, &A_powsum.shape, (A_powsum.default_stride ? NULL : &A_powsum.stride), (A_powsum.default_stride ? NULL : &A_powsum.stride), DT_FP32);
    ...
    __ppl_tensor_info A_shared_2 = {.shape = {1, 64, 1, 2048}, .stride = NULL, .addr = A_shared.addr, .dtype = DT_FP32, .mode = 0, .size = 1, .offset = 0, .unsigned_flag = 0, .default_stride = true};
    __ppl_tensor_info B = {.shape = {1, 64, 1, 2048}, .stride = {4194304, 2048, 2048, 1} , .addr = v4.addr + (((bx * 64)) * 2048+(0) * 1 ) * 4, .dtype = DT_FP32, .mode = 2, .size = 1, .offset = (((bx * 64)) * 2048+(0) * 1 ) * 4, .unsigned_flag = 0, .default_stride = false};
