...
int main() {
  ...
  // 初始化输入数据
  rand_data("test", func_name, 0, input_data, input_size, &input_shape, -5, 5, DT_BFP16);
  ...
  // 保存运行结果
  dump_data("./test", func_name, "_input", ".in", 0, input_data, input_size, DT_BFP16);
  dump_data("./test", func_name, "_output", ".out", 1, output_data, output_size, DT_BFP16);
  delete[] input_data;
  delete[] output_data;
  // 释放计算设备的memory
  ...
  rand_data("test", func_name, 0, input_data, input_size, &input_shape, -5, 5, DT_BFP16);
  // 改为从test中读取输入数据
  //load_file("./test/rms_norm_input0.in", input_data, input_size, input_size, DT_BFP16, DT_BFP16);
