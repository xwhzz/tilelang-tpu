...
int main() {
  ...
  size_t left_size = M * K * DtypeSize(DT_FP16);
  size_t right_size = K * N * DtypeSize(DT_FP16);
  size_t res_size = M * N * DtypeSize(DT_FP16);
#ifdef __bm1690__
  // tpuv7_runimte的状态变量
  tpuRtStatus_t ret;
  // 计算设备runtime初始化
  ...
  // 初始化输入数据
  printf("input_data:\n");
  rand_data("test", "matmul", 0, left_data, left_size, &left_shape, -1.0, 1.0, DT_FP16);
  rand_data("test", "matmul", 1, right_data, right_size, &right_shape, -1.0, 1.0, DT_FP16);
  ...
  int rst = mm2_fp16((unsigned long long)dev_left_data,
  ...
  // 保存运行结果
  dump_data("./test", "matmul", "_left", ".in", 0, left_data, left_size, DT_FP16);
  dump_data("./test", "matmul", "_right", ".in", 1, right_data, right_size, DT_FP16);
  dump_data("./test", "matmul", "_res", ".out", 2, res_data, res_size, DT_FP16);
  ...
  // load_file("./test/matmul_right1.in", right_data, right_size, right_size, DT_FP16, DT_FP16);
  ...
  for (int i = 0; i < WARMUP + REPAET; i++){
    ...
    // int rst = mm2_fp16(bm_mem_get_device_addr(dev_res_data),
    //                        bm_mem_get_device_addr(dev_left_data), bm_mem_get_device_addr(dev_right_data), M, K, N);
    int rst = mm2_fp16(bm_mem_get_device_addr(dev_left_data), bm_mem_get_device_addr(dev_right_data), bm_mem_get_device_addr(dev_res_data));
