...
int main() {
  ...
  size_t left_size = M * K * DtypeSize(DT_FP32);
  size_t right_size = K * N * DtypeSize(DT_FP32);
  ...
  // 初始化输入数据
  printf("input_data:\n");
  rand_data("test", "matmul", 0, left_data, left_size, &left_shape, -1.0, 1.0, DT_FP32);
  rand_data("test", "matmul", 1, right_data, right_size, &right_shape, -1.0, 1.0, DT_FP32);
  ...
  int rst = mm2_fp32((unsigned long long)dev_left_data,
  ...
  // 保存运行结果
  dump_data("./test", "matmul", "_left", ".in", 0, left_data, left_size, DT_FP32);
  dump_data("./test", "matmul", "_right", ".in", 1, right_data, right_size, DT_FP32);
  ...
  for (int i = 0; i < WARMUP + REPAET; i++){
    ...
    int rst = mm2_fp32(bm_mem_get_device_addr(dev_left_data), bm_mem_get_device_addr(dev_right_data), bm_mem_get_device_addr(dev_res_data));
